...
(defcustom ii-irc-directory "~/irc/"
...
(defcustom ii-completing-read 'completing-read
  "Which function to use for channel name completion"
  :group 'ii-mode
  :type '(string))
(defcustom ii-prefered-antishoulder nil
...
(defcustom ii-mode-hooks nil
  "hooks to run on ii-mode"
  :group 'ii-mode
  :type 'hook)

(defcustom ii-ssh-domain nil
...
(defcustom ii-notify-regexps nil
  "A list of regexps to match incoming text for notification"
  :group 'ii-mode
  :type '(string))

(defcustom ii-notify-channels nil
  "A list of channels to recieve special notification love. Uses
  the shortname form \"server/channel\"."
  :group 'ii-mode
  :type '(string))
...
;; censor
(defcustom ii-censor '()
  "A list of basic regexps to match lines to censor from channels"
  :group 'ii-mode
  :type '(string))
...
(make-face 'ii-face-nick-self)
(make-face 'ii-face-date)
(make-face 'ii-face-time)
(make-face 'ii-face-give-voice)
(make-face 'ii-face-take-voice)
(make-face 'ii-face-quit)
(make-face 'ii-face-join)
(make-face 'ii-face-shadow)
(make-face 'ii-face-prompt)
(make-face 'ii-face-msg)
(make-face 'ii-face-msg-self)
(make-face 'ii-face-bold)
(make-face 'ii-face-underline)
(make-face 'ii-face-motd)
(make-face 'ii-face-warning)
(make-face 'ii-face-names)
(make-face 'ii-face-link)
(make-face 'ii-face-mail)
(make-face 'ii-face-hide)

(set-face-attribute 'ii-face-nick nil :foreground "#FF7F24")
(set-face-attribute 'ii-face-nick-self nil :foreground "#B55B1A")
(set-face-attribute 'ii-face-date nil :foreground "#888888")
(set-face-attribute 'ii-face-time nil :foreground "#AAAAAA")
(set-face-attribute 'ii-face-give-voice nil :foreground "#FFFFFF")
(set-face-attribute 'ii-face-take-voice nil :foreground "#00FFFF")
(set-face-attribute 'ii-face-quit nil :foreground "#FF0000")
(set-face-attribute 'ii-face-join nil :foreground "#00FF00")
(set-face-attribute 'ii-face-shadow nil :foreground "#CCCCCC")
(set-face-attribute 'ii-face-prompt nil :foreground "#00FF00")
(set-face-attribute 'ii-face-msg nil :foreground "#FFFFFF")
(set-face-attribute 'ii-face-msg-self nil :foreground "#C0C0B0")
(set-face-attribute 'ii-face-bold nil :bold t)
(set-face-attribute 'ii-face-underline nil :underline t)
(set-face-attribute 'ii-face-motd nil :foreground "#FFFF55")
(set-face-attribute 'ii-face-warning nil :foreground "#FF0000")
(set-face-attribute 'ii-face-names nil :foreground "#FF0000")
(set-face-attribute 'ii-face-link nil :foreground "#AAAAFF" :underline 't)
(set-face-attribute 'ii-face-mail nil :foreground "#AAAAFF" :underline 'nil)
(set-face-attribute 'ii-face-hide nil :foreground (background-color-at-point) :background (background-color-at-point))

(defconst ii-font-lock-keywords
  (list '("^[0-9]+++-[0-9]+-[0-9]+\\ [0-9]+:[0-9]+\\ [A-Za-z0-9].*?$" 0 'default t)
        '("^[0-9]+++-[0-9]+-[0-9]+\\ [0-9]+:[0-9]+\\ \+.*?$" 0 'ii-face-give-voice t)
        '("^[0-9]+++-[0-9]+-[0-9]+\\ [0-9]+:[0-9]+\\ -.*?$" 0 'ii-face-take-voice t)
        '("^[0-9]+++-[0-9]+-[0-9]+\\ [0-9]+:[0-9]+\\ -\\ .*?$" 0 'ii-face-motd t)
        '("^[0-9]+++-[0-9]+-[0-9]+\\ [0-9]+:[0-9]+\\ .*[Uu]nknown\\ [Cc]ommand$" 0 'ii-face-warning t)
        '("^[0-9]+++-[0-9]+-[0-9]+\\ [0-9]+:[0-9]+\\ =\\ .*?$" 0 'ii-face-names t)
        '("^[0-9]+++-[0-9]+-[0-9]+\\ [0-9]+:[0-9]+\\ -!-.*" 0 'ii-face-shadow t)
        '("^[0-9]+++-[0-9]+-[0-9]+\\ [0-9]+:[0-9]+\\ -!-.*quit" 0 'ii-face-take-voice t)
        '("^[0-9]+++-[0-9]+-[0-9]+\\ [0-9]+:[0-9]+\\ -!-.*joined" 0 'ii-face-give-voice t)
        '("(.*" 0 'ii-face-shadow t)
        '("^[0-9]+++-[0-9]+-[0-9]+\\ [0-9]+:[0-9]+\\ \<.+\>.*" 0 'default t)
        '("^[0-9]+++-[0-9]+-[0-9]+\\ [0-9]+:[0-9]+\\ \<.*?\>" 0 'ii-face-nick t)
        '("^[0-9]+++-[0-9]+-[0-9]+\\ [0-9]+:[0-9]+\\ \<\\(mang\\|xmangx\\)> .*" 0 'ii-face-msg-self t)
        '("^[0-9]+++-[0-9]+-[0-9]+\\ [0-9]+:[0-9]+\\ \<\\(mang\\|xmangx\\)\>" 0 'ii-face-nick-self t)
        '("\\(\\.\\|_\\|-\\|\\w\\)+@\\(\\.\\|_\\|-\\|\\w\\)+\\.\\w\\w+" 0 'ii-face-mail t)
        '("\\(http://\\|https://\\|ftp://\\|www\\.\\|ftp\\.\\).[^ <>\"\n$]*" 0 'ii-face-link t)
        '("^[0-9]+++-[0-9]+-[0-9]+\\ [0-9]+:[0-9]+" 0 'ii-face-time t)
        '("^[0-9]+++-[0-9]+-[0-9]+" 0 'ii-face-date t)
        '("\C-b.*?\C-b" 0 'bold prepend)
        '("\C-_.*?\C-_" 0 'underline prepend)
        '("\C-_" 0 'ii-face-hide t)
        '("\C-b" 0 'ii-face-hide t)
        '("^ii>.*" 0 'ii-face-msg t)
        '("^ii>" 0 'ii-face-prompt t)
        '("\C-c[0-9][0-9]" 0 'ii-face-hide t)))
